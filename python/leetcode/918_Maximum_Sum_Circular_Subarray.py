...
class Solution(object):
    def maxSubarraySumCircular(self, A):
        ...
        while i < len(A):
            ...
            ret = max(ret, cur_sum)
            if j-i+1 == length:
                return sum(A) - self.buff(A)
            if cur_sum <= 0:
                keep = min(cur_sum, keep)
                cur_sum = 0
                i = j+1
            j += 1
        return ret
