...
class Solution(object):
    ...
    def move_north(self, length):
        for i in range(1, length+1):
            key = "{0}={1}".format(self.coordinate[0], self.coordinate[1]+1)
    ...
    def move_east(self, length):
        for i in range(1, length+1):
            key = "{0}={1}".format(self.coordinate[0]+1, self.coordinate[1])
            if key in self.obstacles:
                # print(key, "=====")
                return
            self.coordinate[0] += 1
    ...
    def move(self, length):
        if self.direction == 0:
            self.move_north(length)
        elif self.direction == 1:
            self.move_east(length)
        elif self.direction == 2:
            self.move_south(length)
        else:
            self.move_west(length)
    ...
    def robotSim(self, commands, obstacles):
        ...
        for obstacle in obstacles:
            key = "{0}={1}".format(obstacle[0], obstacle[1])
        ...
        for command in commands:
            if command in [-1, -2]:
                ...
                self.move(command)
                # print(self.coordinate, self.direction, command)
                self.distance = max(self.coordinate[0]**2+self.coordinate[1]**2, self.distance)
...
def main():
    ...
    print(s.robotSim(commands, obstacles))
    # print(s.robotSim([9], []))
    # print(s.robotSim([-1, 9], []))
    # print(s.robotSim([-2, 9], []))
    # print(s.robotSim([-2, -2, 9], []))
